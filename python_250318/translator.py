from config import (
    ...
    exit_label, exit_label_stack, update_label, update_label_stack,
...
def translate_to_assembly(js_code):
    ...
    def process_node(node):
        global garbage_address, discriminant_name, array_name, variable_declarations, \
            ...
            matrix_type_list, object_type_list, variable_type_list, array_type_list, string_type_list, \
            variable_kind, \
            exit_label, break_label, update_label, exit_label_stack, update_label_stack
        ...
        match node.type:          
                            ...
                            case "==" | "!=" | "<" | ">" |"<=" | ">=":
                                ...
                                match operator_name:
                                    ...
                                    case "<":
                                        ...
                                        nodes_code.append(f"        ld de, 0                ; assume condition is ok")
                                        nodes_code.append(f"        xor a                   ;")
                                        nodes_code.append(f"        sbc hl, bc              ;")
                ...
                match function_name:
                    ...
                    case _:
                        if node.callee.name:
                            ...
                            # extract returned value: take advantage that JS can only return one value
                            #print (function_name, returns_in_function[function_name])
                            if returns_in_function[function_name] == True:
                                nodes_code.append(f"                                ; get Return value")
                                nodes_code.append(f"        pop de                  ; <<< pop returned value")
                                nodes_code.append(f"        pop bc                  ; <<< pop returned address")
                            
                            # clean stack
                            for arg in node.arguments:
                                nodes_code.append(f"                                ; clean arguments from stack")
                                nodes_code.append(f"        pop hl                  ; <<< pop, clean stack")
                                nodes_code.append(f"        pop hl                  ; <<< pop, clean stack")
                            
                            # restore returned value in stack
                            if returns_in_function[function_name]  == True:
                                nodes_code.append(f"                                ; push Return value to stack again")
                                nodes_code.append(f"        push bc                 ; >>> push returned value")
                                nodes_code.append(f"        push de                 ; >>> push returned address")
                    ...
                    if add_underscore_to_var(object_name) in string_type_list:
                        ...
                        match property_name: 
                            case "charAt" | "charCodeAt":
                                ...
                                nodes_code.append(f"        ld (hl), 1              ; set length = 1")
                                nodes_code.append(f"        inc hl                  ;")
            ...
            case "ForStatement":
                ...
                # Ensure we are pushing before popping later
                exit_label_stack.append(exit_label)
                update_label_stack.append(update_label)
                ...
                if node.test:
                    ...
                    process_node(node.test)
                    nodes_code.append(f"        pop de                      ;")
                    nodes_code.append(f"        xor a                       ;")
                    nodes_code.append(f"        cp e                        ;")
                    nodes_code.append(f"        jp z, {exit_label_stack[-1]}           ; jump to current exit label if false")
            
                if node.body:
                    nodes_code.append(f"                                ; ({node.type}) 3. body -------------")
                    process_node(node.body)
            
                # recover outer label afted processing body
                exit_label = exit_label_stack.pop()
                update_label = update_label_stack.pop()
            ...
            case "FunctionDeclaration": # Function Call, step 2: function tag, pop parameters
                ...
                # 3) 'params' section
                # 3.1) Re-push all parameters in re
                nodes_code.append(f"                                ; ({node.type}) re-push all parameters in reverse")
                nodes_code.append(f"        ld hl, 0                ; get SP value to run down the stack")
                nodes_code.append(f"        add hl, sp              ; HL = SP")
                
                for param in reversed(node.params): # Handle parameters (if any) for the function
                    # pending: it's only for integers     
                    nodes_code.append(f"        ld c, (hl)              ; re-push parameter value")
                    nodes_code.append(f"        inc hl                  ;")
                    nodes_code.append(f"        ld b, (hl)              ;")
                    nodes_code.append(f"        inc hl                  ;")
                    nodes_code.append(f"        push bc                 ;")
                    nodes_code.append(f"        ld c, (hl)              ; re-push parameter address")
                    nodes_code.append(f"        inc hl                  ;")
                    nodes_code.append(f"        ld b, (hl)              ;")
                    nodes_code.append(f"        inc hl                  ;")
                    nodes_code.append(f"        push bc                 ;\n")
                ...
                for param in (node.params):
                    ...
                    if parameter_name not in variable_type_list:
                        ...
                        nodes_code.append(f"        pop bc                  ; <<< pop address")
                ...
                # if no return statement, push bogus values to stack
                #if not any(stmt.type == "ReturnStatement" for stmt in node.body.body):
                #    nodes_code.append(f"        ld hl, 0                ; ({node.type}) if no ReturnStatement") # we could even skip this instruction
                #    nodes_code.append(f"        push hl                 ; >>> push bogus address")
                #    nodes_code.append(f"        push hl                 ; >>> push bogus value\n")
            ...
            case "BreakStatement":
                #print (node)
                nodes_code.append(f"        jp {exit_label}              ; ({node.type})")
                nodes_code.append("")
