from config import (
    ...
    string_max_length,
    matrix_type_list, object_type_list, variable_type_list, array_type_list,
)
from utils import *
import esprima
...
def translate_to_assembly(js_code):
    ...
    def process_node(node):
        global garbage_address
        global discriminant_name
        global array_name
        global variable_declarations
        global function_end_tag
        global function_exit_tag
        global matrix_type_list
        global object_type_list
        global variable_type_list
        global array_type_list
        global variable_kind
        ...
        match node.type:
            ...
            case "BinaryExpression":
                ...
                if node.left.name:
                    ...
                    value_type= "int" # pending...not correct
                    ...
                    case "str":
                        ...
                        match operator_name:
                            case "==":
                                ...
                                nodes_code.append(f"        ld c, 0                 ; assume the strings are different")
            ...
            case "CallExpression":
                ...
                match function_name:
                    ...
                    case _:
                        ...
                        # console.log() call to print text
                        if object_name == "console" and property_name == "log":
                            if node.arguments:
                        ...
                        # Math object: read filenames and add them to the includes list
                        ...
                        # read keyboard
                            ...
                            if event_name== "keydown":
                                ...
                                variable_declarations.append(f'{keypressed_event_name}state defw 1               ; ({node.type}) add/remove flag')
                                #variable_declarations.append(f'{keypressed_event_name}key defw 0                 ; ({node.type}) last key pressed')
                                ...
                                nodes_code.append(f"        jr rea_key              ; loop while eventListener is active")
                                nodes_code.append ("")
            ...
            case "FunctionDeclaration": # Function Call, step 2: function tag, pop parameters
                ...
                # 2) return address
                nodes_code.append(f"        pop hl                  ; <<< return address")
                nodes_code.append(f"        ld (sta_ck2), hl        ; save return address")
                nodes_code.append(f"        ld hl, sta_ck2          ; needed in case another function comes in")
                nodes_code.append(f"        inc (hl)                ; update stack2 pointer")
                nodes_code.append(f"        inc (hl)                ;")
                ...
                for param in reversed(node.params): # Handle parameters (if any) for the function
                    ...
                    if parameter_name not in variable_type_list:
                        ...
                        nodes_code.append(f"        pop de                  ; <<< pop parameter {parameter_name}")
                        nodes_code.append(f"        pop bc                  ; <<< pop address {parameter_name}, unused")
                ...
                nodes_code.append(f"        dec (hl)                ; update pointer")
                nodes_code.append(f"        dec (hl)                ;")
                nodes_code.append(f"        ld hl, (sta_ck2)        ; save return address")
                nodes_code.append(f"        push hl                 ; >>> return address\n")
            ...
            case "ReturnStatement": # Function Call, step 3: return values
                if node.argument:
                    # Process the arguments of the return statement (the expression to return)
                    process_node(node.argument)  # Evaluate the expression
                
                # assembly code
                #nodes_code.append(f"{function_exit_tag} ret                     ; ({node.type}) return from function")
                #nodes_code.append("")
            ...
            case "Literal":
                ...
                if isinstance(literal_value, int): # this works for both integer and boolean
                    ...
                    nodes_code.append(f"        ld de, {literal_value}                ; collect value")
                    nodes_code.append(f"        push hl                 ; >>> push garbage zone address")
                ...
                if isinstance(node.value, str):
                    ...
                    # assembly code
                    nodes_code.append(f"        ld hl, {literal_name}          ; ({node.type}) push literal string pointer")
                    nodes_code.append(f"        push hl                 ; >>> push literal string address")
                    nodes_code.append(f"        ld e, (hl)              ; save value in garbage zone")
                    nodes_code.append(f"        inc hl                  ;")
                    nodes_code.append(f"        ld d, (hl)              ;")
                    nodes_code.append(f"        push de                 ; >>> push value")
            ...
            case "MemberExpression":
                ...
                # object, dictionary        
                    ...
                    if object_type_list.get(object_name):
                        ...
                        dict_not_found_label= new_label("dnf_")
                        ...
                        nodes_code.append(f"        jr {dict_not_found_label}              ;")
                        ...
                        nodes_code.append(f"        push ix                 ; >>> value address")
                        nodes_code.append(f"        pop bc                  ; <<< pop value address")
                        nodes_code.append(f"{dict_not_found_label} push bc                 ; >>> push value address")
                        nodes_code.append(f"        push de                 ; >>> push value\n")
                    ...
                    if array_type_list.get(object_name):
                        ...
                        nodes_code.append(f"        add hl, de              ; array addres + record position") # HL= 2 * ((row# * row_size) + column#) + 2
            ...
            case "VariableDeclarator": # deals with all variable and constant declarations
                ...
                if variable_kind== "const":
                    variable_type_list[variable_name]= "int" # pending: not a variable, but for type it works
                ...
                # strings ---------------------------------------------------------                        
                        ...
                        # variable creation
                        if variable_name not in variable_type_list:
                            variable_type_list[variable_name]= "str"
                            ...
                            variable_declarations.append(f'        ds {filler_size}, 0xff             ;')
                        else:
                            print(f"{node.type}: {variable_name} already declared.")
                                    ...
                                    if callee_type_name == "Array":  
                                        ...
                                        match len(node.init.arguments):
                                            case 1: # 1 dimension array
                                                ...
                                                #variable_type_list[matrix_name]= "int"
                                            case 2: # 2D matrix
